...
import { SEARCHFIELD, MagniferIcon } from '@/constants'
import Divide from './Divide'
import Image from 'next/image'

const SearchBar = () => {
    return (
        <div className="w-184 h-14.5 grid grid-cols-16 items-center rounded-[60px] px-4 border border-coloraccent">
            {SEARCHFIELD.map((f, index) => (
                <div
                    key={index}
                ...
                >
                    <div className='flex justify-center items-center ml-4'>
                        <Input
                            ...
                            type={f.type}
                        />
                        {index !== SEARCHFIELD.length-1 && <Divide style={"w-[1px] h-6"} />}
                    </div>
